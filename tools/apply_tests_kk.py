...
RU_EXTRA = set('врач мама папа лицо летом зимой знакомство имена город работа семья синоним антоним сотни половина суффикс аптека деятеля окончание словам библиотекаря удобный древний старый'.split())
def needs(v):
    # цитаты «...» из казахского текста не считаем: иначе короткий русский хвост («...». Врач — мама.) тонет в них
    ws = W.findall(re.sub(r'«[^»]*»', ' ', v))
    if not ws: return False
    kk = sum(1 for w in ws if KK.search(w)); ru = sum(1 for w in ws if w.lower() in RUW or re.search(r'(ый|ий|ого|его|ому|ему|ться|тся|ать|ять|ние|ция|ции|ых|ами|ями|ешь|ете|ите|ой|ая|ое|ют|ят|ом|ях|[ёэщъ].*)$', w.lower()) or w.lower() in RU_EXTRA)
